...
export default function FormPartes() {
  ...
  const buscarDelitos = async (texto: string) => {
    ...
    const { data } = await supabase
      .from("delitos")
      .select("DELITO_DESAGREGACION_POLICIA_JUDICIAL")
      .ilike("DELITO_DESAGREGACION_POLICIA_JUDICIAL", `%${texto}%`)
      .limit(5);
    setSugerencias(data || []);
  };
            ...
            <div className="flex-[3] space-y-0.5 relative">
              ...
              {sugerencias.length > 0 && (
                <ul className="absolute z-50 w-full bg-neutral-950 border border-white/10 rounded-xl mt-1 shadow-2xl max-h-32 overflow-y-auto custom-scrollbar">
                  {sugerencias.map((s, i) => (
                    <li key={i} onClick={() => { setDelito(s.DELITO_DESAGREGACION_POLICIA_JUDICIAL); setSugerencias([]); }} className="p-2 text-[10px] text-white hover:bg-indigo-600 cursor-pointer border-b border-white/5 last:border-none uppercase transition-colors">
                      {s.DELITO_DESAGREGACION_POLICIA_JUDICIAL}
                    </li>
                  ))}
                </ul>
              )}
